...
const fetchStudents = async () => {
  ...
  if (data) {
    students.value = data.map(s => ({
      ...
      specialCare: s.special_care,
      headacheMedication: s.headache_medication,
      feverMedication: s.fever_medication,
...
const downloadHealthPDF = async () => {
  ...
  if (error) {
    ...
    alert('Error al descargar las fichas médicas');
    return;
  }

  if (!data || data.length === 0) {
    alert('No hay fichas médicas registradas');
    return;
  }
  ...
  doc.setFontSize(18);
  doc.text('Fichas Médicas de Adolescentes', 14, 22);
  
  const tableData = data.map(s => [
    s.student_name,
    s.medication || '-',
    s.special_care || '-',
    s.headache_medication || '-',
    s.fever_medication || '-',
    s.emergency_contact || '-'
  ]);

  autoTable(doc, {
    head: [['Nombre', 'Medicación', 'Cuidado Especial', 'Dolor Cabeza', 'Fiebre', 'Contacto Emergencia']],
    body: tableData,
    startY: 30,
    theme: 'grid',
  ...
  doc.save('fichas-medicas-adolescentes.pdf');
};
  ...
  <div class="container">
    <header class="mb-8 text-center">
      ...
      <div class="header-actions">
        ...
        <button @click="downloadHealthPDF" class="btn-secondary">
          Descargar Fichas Médicas
        </button>
      </div>
    </header>
